from fastapi import APIRouter, Depends, HTTPException, UploadFile, File
from sqlalchemy.orm import Session
from typing import Optional, List
from datetime import datetime
from models.database import get_db, ImageTemplate, ContentTemplate, TemplateState
from pydantic import BaseModel
import json
import random
import os
from pathlib import Path
import shutil
import re
import base64
import uuid
import base64
import mimetypes
import requests
from urllib.parse import urljoin, urlparse
import subprocess
import shlex


def _curl_fetch(url: str, headers: dict = None, as_text: bool = False, timeout_sec: int = 20):
    """Fetch resource via system curl to bypass Python ssl/libressl limitations.
    Returns bytes (default) or text when as_text=True. Raises RuntimeError on failure.
    """
    cmd = ["curl", "-L", "--silent", "--show-error", "--fail", url]
    if headers:
        for k, v in headers.items():
            if v:
                cmd += ["-H", f"{k}: {v}"]
    try:
        proc = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=timeout_sec)
        if proc.returncode != 0:
            raise RuntimeError(proc.stderr.decode("utf-8", errors="ignore") or f"curl exit {proc.returncode}")
        data = proc.stdout
        if as_text:
            return data.decode("utf-8", errors="ignore")
        return data
    except Exception as e:
        raise RuntimeError(str(e))


def _extract_image_url_from_html(html: str, base: str) -> str:
    """Best-effort extract an image URL from HTML content.
    Tries og:image, twitter:image, link rel=image_src, first img[src], and i.ibb.co direct links.
    Returns absolute URL or None.
    """
    try:
        import re as _re
        flags = _re.I | _re.S
        patterns = [
            r'<meta[^>]+property=["\']og:image["\'][^>]+content=["\']([^"\']+)["\']',
            r'<meta[^>]+content=["\']([^"\']+)["\'][^>]+property=["\']og:image["\']',
            r'<meta[^>]+property=["\']og:image:secure_url["\'][^>]+content=["\']([^"\']+)["\']',
            r'<meta[^>]+name=["\']twitter:image["\'][^>]+content=["\']([^"\']+)["\']',
            r'<meta[^>]+content=["\']([^"\']+)["\'][^>]+name=["\']twitter:image["\']',
            r'<link[^>]+rel=["\']image_src["\'][^>]+href=["\']([^"\']+)["\']',
            r'<img[^>]+src=["\']([^"\']+)["\']'
        ]
        for pat in patterns:
            m = _re.search(pat, html, flags)
            if m:
                return urljoin(base, m.group(1))
        # i.ibb.co direct links anywhere in HTML
        m = _re.search(r'https?://i\.ibb\.co/[^\s\'\"]+\.(?:png|jpe?g|webp|gif)', html, flags)
        if m:
            return m.group(0)
        return None
    except Exception:
        return None

router = APIRouter(prefix="/api/template-materials", tags=["template-materials"])

# Request models
class ImageTemplateCreate(BaseModel):
    name: str
    template_type: str  # 'insert' or 'overlay'
    text_style: str
    text_color: str = '#2c3e50'
    background_style: str
    font_size: int = 40
    line_height: str = '1.2'
    mask_opacity: str = '0'
    text_lines: int = 3
    custom_background_path: Optional[str] = None

class ContentTemplateCreate(BaseModel):
    name: str
    description_templates: Optional[List[str]] = []
    use_random_description: bool = True
    no_description: bool = False
    topic_templates: Optional[List[str]] = []
    topic_count: int = 7

class TemplateStateUpdate(BaseModel):
    template_type: str  # 'image' or 'content'
    template_id: Optional[int] = None
    mode: str = 'random'
    enabled: bool = True

@router.get("/image-templates")
async def get_image_templates(db: Session = Depends(get_db)):
    """获取所有图片模板"""
    templates = db.query(ImageTemplate).order_by(ImageTemplate.created_at.desc()).all()
    return [{
        "id": t.id, 
        "name": t.name, 
        "template_type": t.template_type,
        "text_style": t.text_style,
        "text_color": t.text_color,
        "background_style": t.background_style,
        "font_size": t.font_size,
        "line_height": t.line_height,
        "mask_opacity": t.mask_opacity,
        "text_lines": t.text_lines,
        "custom_background_path": t.custom_background_path,
        "created_at": t.created_at.isoformat()
    } for t in templates]

@router.get("/content-templates")  
async def get_content_templates(db: Session = Depends(get_db)):
    """获取所有内容模板"""
    templates = db.query(ContentTemplate).order_by(ContentTemplate.created_at.desc()).all()
    result = []
    for t in templates:
        description_templates = []
        topic_templates = []
        
        if t.description_templates:
            try:
                description_templates = json.loads(t.description_templates)
            except:
                pass
                
        if t.topic_templates:
            try:
                topic_templates = json.loads(t.topic_templates)
            except:
                pass
        
        result.append({
            "id": t.id,
            "name": t.name,
            "description_templates": description_templates,
            "use_random_description": t.use_random_description,
            "no_description": t.no_description,
            "topic_templates": topic_templates,
            "topic_count": t.topic_count,
            "created_at": t.created_at.isoformat()
        })
    
    return result

@router.get("/current-templates")
async def get_current_templates(db: Session = Depends(get_db)):
    """获取当前应用的模板模式"""
    state = get_or_create_template_state(db)
    
    image_template_name = "随机模式"
    content_template_name = "随机模式"
    
    if state.image_template_enabled and state.current_image_template_id:
        image_template = db.query(ImageTemplate).filter(ImageTemplate.id == state.current_image_template_id).first()
        if image_template:
            image_template_name = image_template.name
    
    if state.content_template_enabled and state.current_content_template_id:
        content_template = db.query(ContentTemplate).filter(ContentTemplate.id == state.current_content_template_id).first()
        if content_template:
            content_template_name = content_template.name
    
    return {
        "success": True,
        "image_template_mode": {
            "is_random_mode": not state.image_template_enabled,
            "current_template_id": state.current_image_template_id if state.image_template_enabled else None,
            "current_template_name": image_template_name,
            "mode": state.image_template_mode
        },
        "content_template_mode": {
            "is_random_mode": not state.content_template_enabled,
            "current_template_id": state.current_content_template_id if state.content_template_enabled else None,
            "current_template_name": content_template_name,
            "mode": state.content_template_mode
        }
    }

@router.post("/apply-image-template/{template_id}")
async def apply_image_template(template_id: int, db: Session = Depends(get_db)):
    """应用指定的图片模板"""
    template = db.query(ImageTemplate).filter(ImageTemplate.id == template_id).first()
    if not template:
        raise HTTPException(status_code=404, detail="模板未找到")
    
    # 更新模板状态
    state = get_or_create_template_state(db)
    state.current_image_template_id = template_id
    state.image_template_mode = template.template_type  # 'insert' or 'overlay'
    state.image_template_enabled = True
    db.commit()
    
    return {
        "success": True, 
        "message": f"已应用图片模板: {template.name}", 
        "template_id": template_id,
        "mode": template.template_type
    }

@router.post("/exit-current-template")
async def exit_current_template(db: Session = Depends(get_db)):
    """退出当前指定的图片模板"""
    state = get_or_create_template_state(db)
    state.current_image_template_id = None
    state.image_template_mode = 'random'
    state.image_template_enabled = False
    db.commit()
    
    return {"success": True, "message": "已切换为随机模式"}

@router.get("/get-template-status")
async def get_template_status(db: Session = Depends(get_db)):
    """获取当前模板指定状态"""
    state = get_or_create_template_state(db)
    
    has_template = state.image_template_enabled and state.current_image_template_id is not None
    template_name = "随机模式"
    template_id = None
    
    if has_template:
        template = db.query(ImageTemplate).filter(ImageTemplate.id == state.current_image_template_id).first()
        if template:
            template_name = template.name
            template_id = template.id
    
    return {
        "success": True,
        "has_template": has_template,
        "template_name": template_name,
        "template_id": template_id,
        "mode": state.image_template_mode
    }

@router.post("/apply-content-template/{template_id}")
async def apply_content_template(template_id: int, db: Session = Depends(get_db)):
    """应用指定的内容模板"""
    template = db.query(ContentTemplate).filter(ContentTemplate.id == template_id).first()
    if not template:
        raise HTTPException(status_code=404, detail="模板未找到")
    
    # 更新模板状态
    state = get_or_create_template_state(db)
    state.current_content_template_id = template_id
    state.content_template_mode = 'specific'
    state.content_template_enabled = True
    db.commit()
    
    return {
        "success": True, 
        "message": f"已应用内容模板: {template.name}", 
        "template_id": template_id
    }

@router.post("/enable-content-random-mode")
async def enable_content_random_mode(db: Session = Depends(get_db)):
    """启用内容模板随机模式"""
    state = get_or_create_template_state(db)
    state.current_content_template_id = None
    state.content_template_mode = 'random'
    state.content_template_enabled = False
    db.commit()
    
    return {"success": True, "message": "已启用内容模板随机模式"}

@router.get("/get-current-image-template")
async def get_current_image_template(material_id: Optional[int] = None, db: Session = Depends(get_db)):
    """获取当前图片模板（用于发布时应用）"""
    state = get_or_create_template_state(db)
    
    template = None
    
    if state.image_template_enabled and state.current_image_template_id:
        # 使用指定模板
        template = db.query(ImageTemplate).filter(ImageTemplate.id == state.current_image_template_id).first()
    else:
        # 随机选择模板
        templates = db.query(ImageTemplate).all()
        if templates:
            template = random.choice(templates)
    
    if not template:
        return {"success": False, "message": "没有可用的图片模板"}
    
    template_data = {
        "id": template.id,
        "name": template.name,
        "template_type": template.template_type,
        "text_style": template.text_style,
        "text_color": template.text_color,
        "background_style": template.background_style,
        "font_size": template.font_size,
        "line_height": template.line_height,
        "mask_opacity": template.mask_opacity,
        "text_lines": template.text_lines,
        "custom_background_path": template.custom_background_path
    }
    
    return {"success": True, "template": template_data, "mode": state.image_template_mode}

@router.get("/get-current-content-template")
async def get_current_content_template(db: Session = Depends(get_db)):
    """获取当前内容模板（用于发布时应用）"""
    state = get_or_create_template_state(db)
    
    template = None
    
    if state.content_template_enabled and state.current_content_template_id:
        # 使用指定模板
        template = db.query(ContentTemplate).filter(ContentTemplate.id == state.current_content_template_id).first()
    else:
        # 随机选择模板
        templates = db.query(ContentTemplate).all()
        if templates:
            template = random.choice(templates)
    
    if not template:
        return {"success": False, "message": "没有可用的内容模板"}
    
    # 解析JSON字段
    description_templates = []
    topic_templates = []
    
    if template.description_templates:
        try:
            description_templates = json.loads(template.description_templates)
        except:
            description_templates = []
    
    if template.topic_templates:
        try:
            topic_templates = json.loads(template.topic_templates)
        except:
            topic_templates = []
    
    return {
        "success": True,
        "template": {
            "id": template.id,
            "name": template.name,
            "description_templates": description_templates,
            "use_random_description": template.use_random_description,
            "no_description": template.no_description,
            "topic_templates": topic_templates,
            "topic_count": template.topic_count
        }
    }

# 新增API端点
@router.post("/save-image-template")
async def save_image_template(template: ImageTemplateCreate, db: Session = Depends(get_db)):
    """保存图片模板"""
    try:
        db_template = ImageTemplate(
            name=template.name,
            template_type=template.template_type,
            text_style=template.text_style,
            text_color=template.text_color,
            background_style=template.background_style,
            font_size=template.font_size,
            line_height=template.line_height,
            mask_opacity=template.mask_opacity,
            text_lines=template.text_lines,
            custom_background_path=template.custom_background_path
        )
        
        db.add(db_template)
        db.commit()
        db.refresh(db_template)
        
        return {
            "success": True, 
            "message": f"图片模板 '{template.name}' 保存成功",
            "template_id": db_template.id
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"保存模板失败: {str(e)}")

@router.delete("/image-template/{template_id}")
async def delete_image_template(template_id: int, db: Session = Depends(get_db)):
    """删除图片模板"""
    template = db.query(ImageTemplate).filter(ImageTemplate.id == template_id).first()
    if not template:
        raise HTTPException(status_code=404, detail="模板未找到")
    
    # 如果是当前应用的模板，先退出
    state = get_or_create_template_state(db)
    if state.current_image_template_id == template_id:
        state.current_image_template_id = None
        state.image_template_enabled = False
    
    template_name = template.name
    db.delete(template)
    db.commit()
    
    return {"success": True, "message": f"模板 '{template_name}' 已删除"}

@router.post("/save-content-template")
async def save_content_template(template: ContentTemplateCreate, db: Session = Depends(get_db)):
    """保存内容模板"""
    try:
        db_template = ContentTemplate(
            name=template.name,
            description_templates=json.dumps(template.description_templates, ensure_ascii=False),
            use_random_description=template.use_random_description,
            no_description=template.no_description,
            topic_templates=json.dumps(template.topic_templates, ensure_ascii=False),
            topic_count=template.topic_count
        )
        
        db.add(db_template)
        db.commit()
        db.refresh(db_template)
        
        return {
            "success": True,
            "message": f"内容模板 '{template.name}' 保存成功",
            "template_id": db_template.id
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"保存内容模板失败: {str(e)}")

@router.delete("/content-template/{template_id}")
async def delete_content_template(template_id: int, db: Session = Depends(get_db)):
    """删除内容模板"""
    template = db.query(ContentTemplate).filter(ContentTemplate.id == template_id).first()
    if not template:
        raise HTTPException(status_code=404, detail="模板未找到")
    
    # 如果是当前应用的模板，先退出
    state = get_or_create_template_state(db)
    if state.current_content_template_id == template_id:
        state.current_content_template_id = None
        state.content_template_enabled = False
    
    template_name = template.name
    db.delete(template)
    db.commit()
    
    return {"success": True, "message": f"内容模板 '{template_name}' 已删除"}

@router.post("/upload-background")
async def upload_background(file: UploadFile = File(...), db: Session = Depends(get_db)):
    """上传自定义背景图片"""
    if not file.content_type.startswith('image/'):
        raise HTTPException(status_code=400, detail="请上传图片文件")
    
    # 创建上传目录
    upload_dir = Path("app/static/uploads/backgrounds")
    upload_dir.mkdir(parents=True, exist_ok=True)
    
    # 生成唯一文件名
    file_extension = Path(file.filename).suffix
    filename = f"{datetime.now().strftime('%Y%m%d_%H%M%S')}_{random.randint(1000, 9999)}{file_extension}"
    file_path = upload_dir / filename
    
    try:
        with open(file_path, "wb") as buffer:
            shutil.copyfileobj(file.file, buffer)
        
        # 返回相对路径
        relative_path = f"/static/uploads/backgrounds/{filename}"
        
        return {
            "success": True,
            "message": "背景图片上传成功",
            "file_path": relative_path
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"上传失败: {str(e)}")

class ContentPreviewRequest(BaseModel):
    template_id: Optional[int] = None

@router.post("/generate-content-preview")
async def generate_content_preview(request: ContentPreviewRequest = ContentPreviewRequest(), db: Session = Depends(get_db)):
    """生成内容预览"""
    template_id = request.template_id if request else None
    
    if template_id:
        template = db.query(ContentTemplate).filter(ContentTemplate.id == template_id).first()
        if not template:
            raise HTTPException(status_code=404, detail="模板未找到")
    else:
        # 获取当前模板或随机选择
        state = get_or_create_template_state(db)
        if state.content_template_enabled and state.current_content_template_id:
            template = db.query(ContentTemplate).filter(ContentTemplate.id == state.current_content_template_id).first()
        else:
            templates = db.query(ContentTemplate).all()
            if not templates:
                return {"success": False, "message": "没有可用的内容模板"}
            template = random.choice(templates)
    
    # 生成预览内容 - 使用固定示例标题，不显示地区标签
    preview_content = await generate_template_content(template, "示例标题：9月6日招聘信息", for_preview=True)
    
    return {
        "success": True,
        "content": preview_content,
        "template_name": template.name
    }

# 辅助函数
def get_or_create_template_state(db: Session) -> TemplateState:
    """获取或创建模板状态"""
    state = db.query(TemplateState).first()
    if not state:
        state = TemplateState()
        db.add(state)
        db.commit()
        db.refresh(state)
    return state

def extract_region_from_title(title: str) -> str:
    """严格按照“年”和“央国企”之间的文字作为地区。
    - 首次尝试：原始标题
    - 第二次尝试：标准化标题（去空白、标准括号）
    - 仍未匹配：抛出异常（不再回退“全国”）
    """
    pattern = r"年(.+?)央国企"

    def _clean(s: str) -> str:
        s = s.replace('（', '(').replace('）', ')')
        return s

    m = re.search(pattern, title)
    if m:
        return m.group(1).strip()

    norm = _clean(title)
    m = re.search(pattern, norm)
    if m:
        return m.group(1).strip()

    raise ValueError(f"无法从标题中提取地区（需满足‘年...央国企’）：{title}")

async def generate_template_content(template: ContentTemplate, material_title: str, for_preview: bool = False) -> str:
    """根据模板生成内容"""
    content_parts = []
    
    # 解析模板数据
    topic_templates = []
    if template.topic_templates:
        try:
            topic_templates = json.loads(template.topic_templates)
        except:
            pass
    
    # 生成话题
    topics = []
    if topic_templates:
        def _norm_topic(t: str) -> str:
            s = str(t).replace('\u3000', ' ').strip()
            while s.startswith('#'):
                s = s[1:].strip()
            return f'#{s}' if s else ''
        if for_preview:
            # 预览时显示所有话题（最多显示前10个），并规范化去空格
            for t in topic_templates[:10]:
                nt = _norm_topic(t)
                if nt and nt not in topics:
                    topics.append(nt)
        else:
            # 发布时从模板中随机选择话题，并规范化
            selected_topics = random.sample(topic_templates, min(template.topic_count, len(topic_templates)))
            for t in selected_topics:
                nt = _norm_topic(t)
                if nt and nt not in topics:
                    topics.append(nt)
    
    # 添加固定的地区相关话题（仅发布时添加，预览时不添加）
    if not for_preview:
        region = extract_region_from_title(material_title)
        topics.append(f"#{region}国企")
        topics.append(f"#{region}国企招聘")
    
    # 组合内容
    if not template.no_description:
        description_templates = []
        if template.description_templates:
            try:
                description_templates = json.loads(template.description_templates)
            except:
                pass
        
        if description_templates:
            # 直接使用所有描述内容，不随机选择
            content_parts.extend(description_templates)
    
    # 添加话题
    if topics:
        content_parts.append(' '.join(topics))
    
    return '\n\n'.join(content_parts) if content_parts else ''

def parse_material_title_for_image_text(title: str, text_lines: int) -> List[str]:
    """解析素材标题，生成图片文字内容"""
    lines = []
    
    # 提取日期
    date_match = re.search(r'(\d+月\d+日|\d+\/\d+)', title)
    if date_match:
        lines.append(date_match.group(1))
    
    # 提取地区和企业类型
    region = extract_region_from_title(title)
    enterprise_types = ['央国企', '央企', '国企', '外企', '民企']
    enterprise_type = '央国企'  # 默认
    
    for etype in enterprise_types:
        if etype in title:
            enterprise_type = etype
            break
    
    lines.append(f"{region}{enterprise_type}")
    lines.append("招聘信息差")
    
    # 如果是4行模式，添加引导文字
    if text_lines > 3:
        lines.append("右划更多👉🏻")
    
    return lines[:text_lines]

class GenerateImageRequest(BaseModel):
    template_config: dict
    text_lines: List[str]
    mode: str = 'insert'
    output_path: str

@router.post("/generate-image-from-canvas")
async def generate_image_from_canvas(request: GenerateImageRequest):
    """接收前端Canvas生成的图片数据并保存到指定路径"""
    try:
        # 这个API主要用作前端和后端之间的桥梁
        # 实际的图片生成会在前端完成，然后通过 save-generated-image API保存
        
        return {
            "success": True,
            "message": "请使用前端Canvas生成图片后调用save-generated-image API",
            "config": request.template_config
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"生成图片失败: {str(e)}")

class SaveGeneratedImageRequest(BaseModel):
    image_data: str  # Base64图片数据
    output_path: str  # 输出路径
    filename: Optional[str] = None  # 可选的文件名

@router.post("/save-generated-image") 
async def save_generated_image(request: SaveGeneratedImageRequest):
    """保存前端生成的Base64图片数据到指定路径"""
    try:
        # 解析Base64图片数据
        if ',' in request.image_data:
            # 移除data:image/png;base64,前缀
            base64_data = request.image_data.split(',')[1]
        else:
            base64_data = request.image_data
            
        # 解码图片数据
        image_bytes = base64.b64decode(base64_data)
        
        # 确保输出目录存在
        output_dir = Path(request.output_path)
        output_dir.mkdir(parents=True, exist_ok=True)
        
        # 生成文件名
        if request.filename:
            filename = request.filename
        else:
            filename = f"generated_{uuid.uuid4().hex[:8]}.png"
            
        # 确保文件扩展名为.png
        if not filename.endswith('.png'):
            filename += '.png'
            
        # 保存图片文件
        output_file = output_dir / filename
        with open(output_file, 'wb') as f:
            f.write(image_bytes)
            
        return {
            "success": True,
            "message": f"图片已保存: {filename}",
            "file_path": str(output_file),
            "filename": filename
        }
        
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"保存图片失败: {str(e)}")

# 通过后端代理获取远程图片并返回 data URL，避免前端CORS问题
class FetchImageRequest(BaseModel):
    url: str


@router.post("/fetch-image-dataurl")
async def fetch_image_dataurl(req: FetchImageRequest):
    try:
        url = (req.url or '').strip()
        if not url or not (url.startswith('http://') or url.startswith('https://') or url.startswith('data:image')):
            raise HTTPException(status_code=400, detail="仅支持 http/https 或 data:image URL")

        if url.startswith('data:image'):
            # 直接返回
            return {"success": True, "data_url": url}

        headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0 Safari/537.36',
            'Accept': 'image/avif,image/webp,image/apng,image/svg+xml,image/*,*/*;q=0.8',
            'Accept-Language': 'zh-CN,zh;q=0.9,en;q=0.8',
            'Referer': 'https://google.com/'
        }
        # 针对 ibb.co 域名优化 Referer
        try:
            host = urlparse(url).hostname or ''
            if host.endswith('ibb.co') or host.endswith('i.ibb.co'):
                headers['Referer'] = 'https://ibb.co/'
        except Exception:
            pass

        try:
            resp = requests.get(url, headers=headers, timeout=15, allow_redirects=True)
            ok = resp.ok
        except Exception as e:
            ok = False
            resp = None
            first_error = str(e)
        if not ok:
            # 回退到 curl 抓取（根据URL后缀判断二进制或HTML）
            try:
                guessed, _ = mimetypes.guess_type(url)
                is_image = bool(guessed and guessed.startswith('image/'))
                if is_image:
                    bin_bytes = _curl_fetch(url, headers=headers, as_text=False)
                    class _RespBin:
                        content = bin_bytes
                        text = ''
                        headers = {'Content-Type': guessed}
                    resp = _RespBin()
                else:
                    html_text = _curl_fetch(url, headers=headers, as_text=True)
                    class _RespHtml:
                        text = html_text
                        content = html_text.encode('utf-8', errors='ignore')
                        headers = {'Content-Type': 'text/html; charset=utf-8'}
                    resp = _RespHtml()
            except Exception as e2:
                detail = first_error if 'first_error' in locals() else ''
                raise HTTPException(status_code=500, detail=f"代理获取图片失败: {detail or str(e2)}")

        ctype = (resp.headers.get('Content-Type') or '').lower()
        # 如果返回的是网页而不是图片，尝试从HTML提取直链（如 og:image）
        if ctype.startswith('text/html') or 'text/html' in ctype:
            html = resp.text or ''
            img_url = _extract_image_url_from_html(html, url)
            if not img_url:
                raise HTTPException(status_code=400, detail="URL不是直链图片，且未能从页面提取到图片地址")

            # 使用页面URL作为Referer再次拉取图片直链
            img_headers = dict(headers)
            img_headers['Referer'] = url
            try:
                img_resp = requests.get(img_url, headers=img_headers, timeout=15, allow_redirects=True)
                if not img_resp.ok:
                    raise RuntimeError(f"HTTP {img_resp.status_code}")
                content = img_resp.content
                ctype2 = (img_resp.headers.get('Content-Type') or '').lower()
            except Exception:
                # curl 回退
                try:
                    content = _curl_fetch(img_url, headers=img_headers, as_text=False)
                    ctype2 = mimetypes.guess_type(img_url)[0] or 'image/jpeg'
                except Exception as e3:
                    raise HTTPException(status_code=500, detail=f"图片直链拉取失败: {str(e3)}")
            if not ctype2.startswith('image/'):
                # 根据URL后缀猜测
                guess, _ = mimetypes.guess_type(img_url)
                ctype2 = guess if (guess and guess.startswith('image/')) else 'image/jpeg'
            b64 = base64.b64encode(content).decode('utf-8')
            data_url = f"data:{ctype2};base64,{b64}"
            return {"success": True, "data_url": data_url}
        else:
            # 已是图片内容
            content = resp.content if hasattr(resp, 'content') else b''
            if not content:
                # curl 回退
                try:
                    content = _curl_fetch(url, headers=headers, as_text=False)
                except Exception as e4:
                    raise HTTPException(status_code=500, detail=f"图片获取失败: {str(e4)}")
            if not ctype or not ctype.startswith('image/'):
                guess, _ = mimetypes.guess_type(url)
                ctype = guess if (guess and guess.startswith('image/')) else 'image/jpeg'
            b64 = base64.b64encode(content).decode('utf-8')
            data_url = f"data:{ctype};base64,{b64}"
            return {"success": True, "data_url": data_url}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"代理获取图片失败: {str(e)}")

@router.post("/generate-template-image-for-material")
async def generate_template_image_for_material(
    material_id: int,
    default_mode: Optional[str] = None,
    db: Session = Depends(get_db)
):
    """为指定素材生成模版图片"""
    try:
        print(f"\n🎯 DEBUG: 开始为素材 {material_id} 生成模版图片")
        
        # 获取素材信息
        from models.database import XiaohongshuMaterial
        material = db.query(XiaohongshuMaterial).filter(XiaohongshuMaterial.id == material_id).first()
        if not material:
            print(f"❌ DEBUG: 素材 {material_id} 不存在")
            raise HTTPException(status_code=404, detail="素材不存在")
        
        print(f"✅ DEBUG: 找到素材 - ID:{material_id}, 标题:'{material.title}'")
            
        # 获取当前模版状态
        template_state = get_or_create_template_state(db)
        print(f"📊 DEBUG: 模版状态 - 启用:{template_state.image_template_enabled}, 模版ID:{template_state.current_image_template_id}, 模式:{template_state.image_template_mode}")
        
        # 获取图片模版
        template = None
        # 如果已指定模板，则忽略默认模式
        if template_state.image_template_enabled and template_state.current_image_template_id:
            print(f"🔍 DEBUG: 使用指定模版，查找模版ID: {template_state.current_image_template_id}")
            template = db.query(ImageTemplate).filter(
                ImageTemplate.id == template_state.current_image_template_id
            ).first()
            if template:
                print(f"✅ DEBUG: 找到指定模版 - ID:{template.id}, 名称:'{template.name}', 类型:{template.template_type}")
            else:
                print(f"❌ DEBUG: 未找到指定的模版ID: {template_state.current_image_template_id}")
        else:
            # 未指定模板时，根据 default_mode 选择
            mode = (default_mode or '').strip().lower()
            insert_templates = db.query(ImageTemplate).filter(ImageTemplate.template_type == 'insert').all()
            overlay_templates = db.query(ImageTemplate).filter(ImageTemplate.template_type == 'overlay').all()
            print(f"📋 DEBUG: 插入模板数: {len(insert_templates)} 覆盖模板数: {len(overlay_templates)} 默认模式: {mode}")

            def pick_random(lst):
                return random.choice(lst) if lst else None

            chosen_type = None
            if mode == 'insert':
                chosen_type = 'insert'
                template = pick_random(insert_templates) or pick_random(overlay_templates)
            elif mode == 'overlay':
                chosen_type = 'overlay'
                template = pick_random(overlay_templates) or pick_random(insert_templates)
            elif mode == 'random':
                available_types = []
                if insert_templates:
                    available_types.append('insert')
                if overlay_templates:
                    available_types.append('overlay')
                if available_types:
                    chosen_type = random.choice(available_types)
                    template = pick_random(insert_templates if chosen_type == 'insert' else overlay_templates)
                else:
                    # 兜底：任意模板
                    all_templates = db.query(ImageTemplate).all()
                    template = pick_random(all_templates)
            else:
                # 未传默认模式：保持兼容行为，任意随机
                all_templates = db.query(ImageTemplate).all()
                template = pick_random(all_templates)

            if template:
                print(f"🎲 DEBUG: 选择模版 - ID:{template.id}, 名称:'{template.name}', 类型:{template.template_type}")
            else:
                print("❌ DEBUG: 没有可用的模版")
                
        if not template:
            print(f"❌ DEBUG: 最终没有找到可用的模版")
            return {"success": False, "message": "没有可用的图片模版"}
            
        print(f"🎨 DEBUG: 最终使用模版详情:")
        print(f"   - ID: {template.id}")
        print(f"   - 名称: '{template.name}'")
        print(f"   - 类型: {template.template_type}")
        print(f"   - 文字样式: {template.text_style}")
        print(f"   - 背景样式: {template.background_style}")
        print(f"   - 字体大小: {template.font_size}")
        print(f"   - 文字颜色: {template.text_color}")
            
        # 解析素材标题生成文字内容
        text_lines = parse_material_title_for_image_text(material.title, template.text_lines)
        print(f"📝 DEBUG: 解析标题生成文字内容: {text_lines}")

        # 覆盖模式：确定目标文件路径（第一张图片），并将原图作为背景（使用 data URL 以便前端可加载）
        target_file_path = None
        overlay_data_url = None
        if template.template_type == 'overlay':
            try:
                folder_path = Path(material.folder_path)
                if folder_path.exists():
                    image_extensions = {'.jpg', '.jpeg', '.png', '.JPG', '.JPEG', '.PNG'}
                    image_files = []
                    for ext in image_extensions:
                        image_files.extend(folder_path.glob(f'*{ext}'))
                    if image_files:
                        image_files.sort(key=lambda x: x.name)
                        first_image = image_files[0]
                        target_file_path = str(first_image)
                        # 读取图片并转为 data URL，避免浏览器无法直接加载本地路径
                        try:
                            ext = first_image.suffix.lower()
                            mime = 'image/png'
                            if ext in ['.jpg', '.jpeg']:
                                mime = 'image/jpeg'
                            elif ext == '.png':
                                mime = 'image/png'
                            with open(first_image, 'rb') as f:
                                b64 = base64.b64encode(f.read()).decode('utf-8')
                            overlay_data_url = f"data:{mime};base64,{b64}"
                        except Exception:
                            overlay_data_url = None
            except Exception:
                target_file_path = None

        # 构建模版配置
        template_config = {
            "text_style": template.text_style,
            "text_color": template.text_color,
            "background_style": template.background_style,
            "font_size": template.font_size,
            "line_height": float(template.line_height),
            "mask_opacity": float(template.mask_opacity),
            # 覆盖模式使用 data URL 作为背景，插入模式使用自定义/预设背景
            "custom_background_path": (overlay_data_url if template.template_type == 'overlay' else template.custom_background_path)
       }
        print(f"⚙️ DEBUG: 构建的模版配置: {template_config}")

        return {
            "success": True,
            "template_config": template_config,
            "text_lines": text_lines,
            "mode": template.template_type,
            "output_path": material.folder_path,
            "target_file_path": target_file_path,
            "material_id": material_id,
            "template_name": template.name
        }
        
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"生成模版图片配置失败: {str(e)}")
